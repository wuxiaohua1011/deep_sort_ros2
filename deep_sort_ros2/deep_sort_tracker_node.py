...
class DeepSORTNode(rclpy.node.Node):
    def __init__(self):
        ...
        self.model_filename = self.get_parameter(
            'model_data_path').get_parameter_value().string_value

        self.encoder = gdet.create_box_encoder(
            self.model_filename, batch_size=1)
        ...
        paths = [
            Path(p)/"deep_sort_ros2/checkpoints/yolov4-416" for p in sys.path if "deep_sort" in p]
        correct_path = None
        for p in paths:
            if p.exists():
    ...
    def on_image_received(self, msg):
        ...
        cv2.imshow("Output Video", cv2.resize(result, dsize=(800, 600)))
        cv2.waitKey(1)
