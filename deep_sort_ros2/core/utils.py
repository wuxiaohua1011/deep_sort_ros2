...
def read_class_names(class_file_name):
    ...
    import sys
    paths = [Path(p) for p in sys.path if "deep_sort" in p]
    correct_file_path = None
    for p in paths:
        if (p / class_file_name).exists():
            correct_file_path = (p / class_file_name).as_posix()
    assert correct_file_path is not None, f"File {class_file_name} not found in these directories [{paths}]"
    with open(correct_file_path, 'r') as data:
        for ID, name in enumerate(data):
            names[ID] = name.strip('\n')
    return names


def load_config(FLAGS, is_tiny=False):
